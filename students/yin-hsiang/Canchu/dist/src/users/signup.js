            ...
            sql.query("SELECT email FROM user WHERE email = ?", [parsedReq.data.email], async function (err, result, fields) {
                if (err) {
                    ...
                    if (emails.length > 0) {
                        ...
                        sql.query("INSERT INTO user (provider, name, email, password) VALUES ('native', ?, ?, ?)", [parsedReq.data.name, parsedReq.data.email, passwd], async function (err, result, fields) {
                            if (err) {
